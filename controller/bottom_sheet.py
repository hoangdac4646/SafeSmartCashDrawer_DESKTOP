...
class KitchenSinkBottomSheet(Screen):
    ...
    def show_example_bottom_sheet(self):
        from kivymd.uix import MDListBottomSheet
    ...
    def show_example_grid_bottom_sheet(self):
        from kivymd.uix import MDGridBottomSheet

        bs_menu = MDGridBottomSheet()
        bs_menu.add_item(
            "Facebook",
            lambda x: self.app.callback_for_menu_items("Facebook"),
            icon_src=f"{os.environ['APP_ASSETS']}facebook-box.png",
        )
        bs_menu.add_item(
            "YouTube",
            lambda x: self.app.callback_for_menu_items("YouTube"),
            icon_src=f"{os.environ['APP_ASSETS']}youtube-play.png",
        )
        bs_menu.add_item(
            "Twitter",
            lambda x: self.app.callback_for_menu_items("Twitter"),
            icon_src=f"{os.environ['APP_ASSETS']}twitter.png",
        )
        bs_menu.add_item(
            "Da Cloud",
            lambda x: self.app.callback_for_menu_items("Da Cloud"),
            icon_src=f"{os.environ['APP_ASSETS']}cloud-upload.png",
        )
        bs_menu.add_item(
            "Camera",
            lambda x: self.app.callback_for_menu_items("Camera"),
            icon_src=f"{os.environ['APP_ASSETS']}camera.png",
        )
        bs_menu.open()
    ...
    def show_example_custom_bottom_sheet(
        ...
        from kivymd.uix import MDCustomBottomSheet
